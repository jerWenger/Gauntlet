...
origins = [0 0; 0 0; 1 0; 0 -2; 1.75 -2.75; 1.5 -1; 0 -2.75];
...
orientations = [0 pi/6 0 pi/3 3*pi/4 3*pi/4 3*pi/4];
...
%     title(['Scan ', num2str(i), ' in Global frame pre-translation']);
    ...
    all_points = [all_points cartesianPointsInGFrame];
end
% display the figure that contains all of the scans
figure(allScansFig);

save('Cartesians.mat','all_points')